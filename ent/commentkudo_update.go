...
func (cku *CommentKudoUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(commentkudo.Table, commentkudo.Columns, sqlgraph.NewFieldSpec(commentkudo.FieldUserID, field.TypeUUID), sqlgraph.NewFieldSpec(commentkudo.FieldCommentID, field.TypeInt))
...
func (ckuo *CommentKudoUpdateOne) sqlSave(ctx context.Context) (_node *CommentKudo, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(commentkudo.Table, commentkudo.Columns, sqlgraph.NewFieldSpec(commentkudo.FieldUserID, field.TypeUUID), sqlgraph.NewFieldSpec(commentkudo.FieldCommentID, field.TypeInt))
	if id, ok := ckuo.mutation.UserID(); !ok {
		return nil, &ValidationError{Name: "user_id", err: errors.New(`ent: missing "CommentKudo.user_id" for update`)}
	} else {
		_spec.Node.CompositeID[0].Value = id
	}
	if id, ok := ckuo.mutation.CommentID(); !ok {
		return nil, &ValidationError{Name: "comment_id", err: errors.New(`ent: missing "CommentKudo.comment_id" for update`)}
	} else {
