...
func (tku *ThreadKudoUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(threadkudo.Table, threadkudo.Columns, sqlgraph.NewFieldSpec(threadkudo.FieldUserID, field.TypeUUID), sqlgraph.NewFieldSpec(threadkudo.FieldThreadID, field.TypeInt))
...
func (tkuo *ThreadKudoUpdateOne) sqlSave(ctx context.Context) (_node *ThreadKudo, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(threadkudo.Table, threadkudo.Columns, sqlgraph.NewFieldSpec(threadkudo.FieldUserID, field.TypeUUID), sqlgraph.NewFieldSpec(threadkudo.FieldThreadID, field.TypeInt))
	if id, ok := tkuo.mutation.UserID(); !ok {
		return nil, &ValidationError{Name: "user_id", err: errors.New(`ent: missing "ThreadKudo.user_id" for update`)}
	} else {
		_spec.Node.CompositeID[0].Value = id
	}
	if id, ok := tkuo.mutation.ThreadID(); !ok {
		return nil, &ValidationError{Name: "thread_id", err: errors.New(`ent: missing "ThreadKudo.thread_id" for update`)}
	} else {
