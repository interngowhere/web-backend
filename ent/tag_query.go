...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withTaggedThreads: tq.withTaggedThreads.Clone(),
...
// WithTaggedThreads tells the query-builder to eager-load the nodes that are connected to
// the "tagged_threads" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TagQuery) WithTaggedThreads(opts ...func(*ThreadQuery)) *TagQuery {
	query := (&ThreadClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTaggedThreads = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withTaggedThreads; query != nil {
		if err := tq.loadTaggedThreads(ctx, query, nodes,
			func(n *Tag) { n.Edges.TaggedThreads = []*Thread{} },
			func(n *Tag, e *Thread) { n.Edges.TaggedThreads = append(n.Edges.TaggedThreads, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadTaggedThreads(ctx context.Context, query *ThreadQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Thread)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.TaggedThreadsTable)
		s.Join(joinT).On(s.C(thread.FieldID), joinT.C(tag.TaggedThreadsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.TaggedThreadsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.TaggedThreadsPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "tagged_threads" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
