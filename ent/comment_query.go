...
func (cq *CommentQuery) QueryCommentKudoes() *CommentKudoQuery {
	query := (&CommentKudoClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withThreads:        cq.withThreads.Clone(),
		withCommentAuthors: cq.withCommentAuthors.Clone(),
		withKudoedUsers:    cq.withKudoedUsers.Clone(),
		withCommentKudoes:  cq.withCommentKudoes.Clone(),
...
func (cq *CommentQuery) WithThreads(opts ...func(*ThreadQuery)) *CommentQuery {
	...
	cq.withThreads = query
	return cq
}

// WithCommentAuthors tells the query-builder to eager-load the nodes that are connected to
// the "comment_authors" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CommentQuery) WithCommentAuthors(opts ...func(*UserQuery)) *CommentQuery {
	query := (&UserClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCommentAuthors = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	var (
		...
		loadedTypes = [4]bool{
			cq.withThreads != nil,
			cq.withCommentAuthors != nil,
			cq.withKudoedUsers != nil,
			cq.withCommentKudoes != nil,
		}
	)
	if cq.withThreads != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withThreads; query != nil {
		if err := cq.loadThreads(ctx, query, nodes, nil,
			func(n *Comment, e *Thread) { n.Edges.Threads = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCommentAuthors; query != nil {
		if err := cq.loadCommentAuthors(ctx, query, nodes, nil,
			func(n *Comment, e *User) { n.Edges.CommentAuthors = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withKudoedUsers; query != nil {
		if err := cq.loadKudoedUsers(ctx, query, nodes,
			func(n *Comment) { n.Edges.KudoedUsers = []*User{} },
			func(n *Comment, e *User) { n.Edges.KudoedUsers = append(n.Edges.KudoedUsers, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCommentKudoes; query != nil {
		if err := cq.loadCommentKudoes(ctx, query, nodes,
			func(n *Comment) { n.Edges.CommentKudoes = []*CommentKudo{} },
			func(n *Comment, e *CommentKudo) { n.Edges.CommentKudoes = append(n.Edges.CommentKudoes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadThreads(ctx context.Context, query *ThreadQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Thread)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].thread_thread_comments == nil {
			continue
		}
		fk := *nodes[i].thread_thread_comments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "thread_thread_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CommentQuery) loadCommentAuthors(ctx context.Context, query *UserQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *User)) error {
...
func (cq *CommentQuery) loadKudoedUsers(ctx context.Context, query *UserQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(comment.KudoedUsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(comment.KudoedUsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(comment.KudoedUsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(comment.KudoedUsersPrimaryKey[1]))
...
func (cq *CommentQuery) loadCommentKudoes(ctx context.Context, query *CommentKudoQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *CommentKudo)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(commentkudo.FieldCommentID)
	}
	query.Where(predicate.CommentKudo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(comment.CommentKudoesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (cq *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withCommentAuthors != nil {
			_spec.Node.AddColumnOnce(comment.FieldCreatedBy)
		}
	}
