...
// WithTopicThreads tells the query-builder to eager-load the nodes that are connected to
// the "topic_threads" edge. The optional arguments are used to configure the query builder of the edge.
...
func (tq *TopicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Topic, error) {
	...
	if query := tq.withTopicThreads; query != nil {
...
func (tq *TopicQuery) loadTopicThreads(ctx context.Context, query *ThreadQuery, nodes []*Topic, init func(*Topic), assign func(*Topic, *Thread)) error {
	...
	query.Where(predicate.Thread(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(topic.TopicThreadsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.topic_topic_threads
		if fk == nil {
			return fmt.Errorf(`foreign-key "topic_topic_threads" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
