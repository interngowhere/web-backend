...
func (cc *CommentCreate) SetCommentAuthorsID(id uuid.UUID) *CommentCreate {
	cc.mutation.SetCommentAuthorsID(id)
	return cc
}
...
func (cc *CommentCreate) SetCommentAuthors(u *User) *CommentCreate {
	return cc.SetCommentAuthorsID(u.ID)
}
...
func (cc *CommentCreate) AddKudoedUserIDs(ids ...uuid.UUID) *CommentCreate {
	cc.mutation.AddKudoedUserIDs(ids...)
	return cc
}
...
func (cc *CommentCreate) check() error {
	...
	if _, ok := cc.mutation.ThreadsID(); !ok {
		return &ValidationError{Name: "threads", err: errors.New(`ent: missing required edge "Comment.threads"`)}
	}
	if _, ok := cc.mutation.CommentAuthorsID(); !ok {
		return &ValidationError{Name: "comment_authors", err: errors.New(`ent: missing required edge "Comment.comment_authors"`)}
	}
	return nil
}
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.ThreadsIDs(); len(nodes) > 0 {
		...
		_node.thread_thread_comments = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := cc.mutation.CommentAuthorsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   comment.CommentAuthorsTable,
			Columns: []string{comment.CommentAuthorsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
	...
	if nodes := cc.mutation.KudoedUsersIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
